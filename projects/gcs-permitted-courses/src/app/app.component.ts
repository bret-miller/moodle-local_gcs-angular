...
export class AppComponent {
  ...
  ngAfterViewInit() {
    ...
    this.codelistsdatasvc.loadCodeLists(this.tbldatasvc.coldefs).subscribe(
      // success
      () => {
        // populate the select list from the cached codelist
  ...
  getFullList() {
    ...
    this.tbldatasvc.getlistbyprogramcode(this.listSel.selected).subscribe(
      ...
      list => {
        ...
        list.sort((t1, t2) => {
          if (t1.categorycode > t2.categorycode) { return -1; }
          if (t1.categorycode < t2.categorycode) { return 1; }
  ...
  onAddClick() {
    this.origRec = this.tbldatasvc.initRec(this.listSel.selected);// "original" rec is a new empty rec
  ...
  openDialog(rec: any) {
    let dialogRef = this.dialog.open(GcsStandardAddUpdRecDlgComponent, {
      ...
      height: '450px',
      data: {
        title: 'Permitted Courses',// dialog title
