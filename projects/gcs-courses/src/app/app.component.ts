...
import { GcsCoursesDataService } from 'services/gcs-courses-data.service';
...
export class AppComponent {
  ...
  constructor(
    ...
    public tbldatasvc: GcsCoursesDataService,
  ...
  // initialization
  ngAfterViewInit() {
    this.sort.sort({ id: 'coursecode', start: 'asc', disableClear: false });// initialize sort

    // build dynamic dropdown lists defined in flddefs
    const bnr = this.gcsdatasvc.showNotification('Loading...', 'Hourglass Top');
    this.codelistsdatasvc.loadDependentCodeLists(this.tbldatasvc.flddefs(), 'tbl_course').subscribe({// also might be needed for dupchecking the coursecode for an add
      // success
      next: () => {
  ...
  onRowClick(rec: any, clickedcol: fldDef) {
    if (clickedcol.datatype !== 'buttons') {
      this.origRec = rec;// copy reference of the rec in model list so we can refresh it upon save
      this.openDialog(rec);
    }
  }
