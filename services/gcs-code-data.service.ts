...
//`;
  ...
  queuegetdependencies(rec: any, queue: any[]) {
    return this.gcsdatasvc.queuegetlist('table_record_dependencies', { tablecode: 'codeset.' + rec.codeset, keycsv: rec.code }, queue);
  }
  ...
  valRec(rec: any, flddefs: fldDef[]) {
    return new Observable<string>((observer) => {
      ...
      this.getlistbycodeset(rec.codeset).subscribe({
        next: (list) => {
          // look through the list to see if the record already exists
          for (let i = 0, dbrec; dbrec = list[i]; i++) {
            if (dbrec.code.toUpperCase() === rec.code.toUpperCase()) {
              // record found on db:  for adds or if a key field has changed, indicate it's a duplicate
              if (dbrec.id != rec.id) {// note that the dbrec.id is a string whereas the rec.id is a number type so the compare cannot be ===
                errmsg = '"' + dbrec.code +
                  '" is already defined in this codeset.';
                break;
              }
            }
          }
  ...
  // NOW DONE AT SERVER BY Data.php in get_codes()
